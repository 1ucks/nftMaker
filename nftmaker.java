...
public class nftmaker {
	public static void main(String[] args) throws IOException, AWTException, InterruptedException {
		...
		boolean con = false;
		// checks to see if you have windows or linux to determine whos pc ur on for the
		// chords
		if (System.getProperty("os.name").toLowerCase().startsWith("windows")) {
			...
			// if its a school computer:
			if (schoolComputer.toLowerCase().startsWith("y")) {
				sCPU = true;
			} else {
				// dectects whos cpu lux or kieran
				System.out
						.print("Who's computer are you on? (Kieran's, lux's, type 'add' to use a different computer) ");
				String cpUser = in.next();
				/* for kieran: */
				if (cpUser.toLowerCase().startsWith("k")) {
					// kierans windows chords
					winX = 1100;
					winY = 200;
					/* for lux: */
				} else if (cpUser.toLowerCase().startsWith("l")) {
					// lux's windows chords
					winX = 1000;
					winY = 200;
					/* for anyone: */
				} else {
			...
			// asks for the nessecary locations
...
//num of images in the pic
		...
		//finds the numIm ammount images and puts them into an array
		//it even filters the images
		for (int i = 0; i < numIm; i++) {
			colorFilter = true;
			//ets the random word and prints it out
			String randomLine = getRandomWord(count);
			System.out.println(randomLine);
			//searches for the images gets em copies em does sfuff
			//explained more in detail later on
			try {
				...
				if (System.getProperty("os.name").toLowerCase().startsWith("windows")) {
					...
					dataArray[i] = data;
					System.out.println(data);
					//^that was all for windows below is for linux
				}else if (System.getProperty("os.name").toLowerCase().startsWith("linux")) {
		...
		//prints the images in data array
		printIm(dataArray);
	}
//Methods, Wooo
	...
	public static void printIm(String datas[]) throws IOException {
		// sets the first image as the background 
	...
	public static boolean badColorFilter(String urL) throws IOException, AWTException {
		...
		// checks if the the color in the top left of the image is dominant in a certain
	...
	public static void getsToGoogleAndPastes(boolean sCPU, String chromePath, Robot robot)
